...
import { etiquetteTemplates } from "@/lib/general-templates";
...
export function EtiquetteWritingPage() {
  const router = useRouter();
  const { refreshCredits } = useCredits();
  const searchParams = useSearchParams();
  const templateTitle = searchParams.get("title") || "礼仪公文";
  const templateId = searchParams.get("template") || "2401";
  const source = searchParams.get("source") || "hot"; // 获取source参数

  // 礼仪公文模块的类型映射函数
  const getEtiquetteTypeByTemplateId = (templateId: string): ConversationType => {
    const mapping: Record<string, ConversationType> = {
      '2401': 'etiquette-thank-you-letter',
      '2402': 'etiquette-congratulation-letter',
      '2403': 'etiquette-condolence-letter',
      '2404': 'etiquette-speech-draft',
    };
    return mapping[templateId] || 'etiquette-thank-you-letter';
  };
  ...
  const getTemplatesFromSource = () => {
    if (source === "hot") {
      ...
      switch (platform) {
        case "etiquette":
          return etiquetteTemplates.map((t: any) => ({
            id: t.id,
            icon: "etiquette",
            iconBg: t.color,
            title: t.title,
  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      try {
        // 根据当前模板ID获取对应的子类型
        const conversationType = getEtiquetteTypeByTemplateId(activeTemplate.toString());
  ...
  const getWelcomeMessage = (templateId: string): string => {
    const welcomeMessages: Record<string, string> = {
      "2401": `您好，我是拥有50年经验的政务公文感谢信撰写专家。请告诉我您需要给谁（单位或个人）写感谢信，是因为什么具体事情（如考察指导、项目支持等），以及您希望传达的核心情感。我将为您字斟句酌，打造一份得体、专业的公文感谢信。`,
      "2402": `您好!我是政务公文祝贺信撰写专家,拥有50年政务公文实战经验,精通各级党政机关公文规范。

请告诉我:**您需要撰写什么类型的祝贺信?发文单位和受文单位分别是什么?**让我们开始为您打造一封得体、规范、有温度的政务祝贺信吧!`,
      "2403": `您好!我是拥有50年政务公文吊唁信撰写经验的专业顾问。

现在,请告诉我您需要撰写什么样的吊唁信?我将竭诚为您服务!`,
      "2404": `您好！我是拥有50年体制内文字工作经验的资深笔杆子，深谙中国政务公文写作规范与领导讲话艺术。

请告诉我您的写作需求（如：是年度总结还是会议致辞？）、听众对象（如：上级领导还是基层群众？）及核心意图。`,
    };
    return welcomeMessages[templateId] || welcomeMessages["2401"];
  };

  // 初始化欢迎消息（所有礼仪公文子类型）
  useEffect(() => {
    const etiquetteTemplateIds = ["2401", "2402", "2403", "2404"];
    if (etiquetteTemplateIds.includes(templateId) && messages.length === 0) {
      setMessages([{
        id: 'welcome',
        role: 'assistant',
  ...
  const getApiEndpoint = (templateId: string): string => {
    switch (templateId) {
      case "2401": return "/api/etiquette/thank-you-letter";
      case "2402": return "/api/etiquette/congratulation-letter";
      case "2403": return "/api/etiquette/condolence-letter";
      case "2404": return "/api/etiquette/speech-draft";
      default: return "/api/etiquette/thank-you-letter";
    }
  };
  ...
  const handleCommunicationNewConversation = () => {
    ...
    // 所有礼仪公文子类型：重置消息列表为对应的欢迎消息
    const etiquetteTemplateIds = ["2401", "2402", "2403", "2404"];
    if (etiquetteTemplateIds.includes(templateId)) {
      setMessages([{
        id: 'welcome-' + Date.now(),
        role: 'assistant',
        content: getWelcomeMessage(templateId),
  ...
  return (
    <div className="flex h-[calc(100vh-56px)]">
      {["2401", "2402", "2403", "2404"].includes(templateId) ? (
        /* 所有礼仪公文子类型：统一使用对话模式UI */

...
import { projectManagementTemplates } from "@/lib/general-templates";
...
export function ProjectManagementWritingPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const templateTitle = searchParams.get("title") || "项目管理";
  const templateId = searchParams.get("template") || "1401";
  const source = searchParams.get("source") || "hot"; // 获取source参数

  // 项目管理模块的类型映射函数
  const getProjectManagementTypeByTemplateId = (templateId: string): ConversationType => {
    const mapping: Record<string, ConversationType> = {
      '1401': 'project-management-startup-report',
      '1402': 'project-management-plan',
      '1403': 'project-management-progress-tracking',
      '1404': 'project-management-communication-plan',
      '1405': 'project-management-acceptance-report',
      '1406': 'project-management-post-evaluation',
    };
    return mapping[templateId] || 'project-management-startup-report';
  };
    ...
    if (source === "hot") {
      ...
      switch (platform) {
        case "project-management":
          return projectManagementTemplates.map((t: any) => ({
            id: t.id,
  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      try {
        // 根据当前模板ID获取对应的子类型
        const conversationType = getProjectManagementTypeByTemplateId(activeTemplate.toString());
  ...
  const getWelcomeMessage = (templateId: string): string => {
    ...
    const welcomeMessages: Record<string, string> = {
      "1401": `👋 您好!我是您的项目启动报告专家,拥有50年项目管理实战经验,专注于帮助您成功启动各类项目。
...
**现在,请告诉我:**
- 您的项目类型是什么?(如:IT系统、工程建设、产品研发等)
- 项目的大致规模和预算范围?
- 您目前最需要帮助的具体方面是什么?

让我们开始打造您的专业项目启动方案吧!🚀`,
      "1402": `您好!👋 我是您的专属项目计划书大师,拥有50年的项目落地实战经验。

### 我能为您提供以下服务:

✅ **商业计划书撰写** - 融资、招商、合作必备
✅ **项目实施方案** - IT项目、工程项目、产品开发
✅ **可行性研究报告** - 投资决策的科学依据
✅ **项目管理计划** - WBS分解、进度规划、资源配置
✅ **风险管理方案** - 识别风险、制定应对策略

### 我的工作流程:

1. **需求探索** → 深入了解您的项目背景和目标
2. **信息收集** → 获取必要的项目资料和约束条件
3. **方案设计** → 构建项目计划框架并确认
4. **内容撰写** → 分模块撰写专业文档
5. **审查优化** → 全面检查并根据反馈完善
6. **交付指导** → 提供Markdown文档及执行建议

现在,请告诉我:**您需要撰写什么类型的项目计划书?**

我会根据您的需求,为您打造专业、实用、可落地的解决方案! 🚀`,
      "1403": `你好，我是拥有50年经验的项目进度跟踪大师。请把你的项目难题交给我，无论是从零规划，还是中途拯救失控的进度，我都能为你提供如手术刀般精准的方案。

**我的工作流程:**
1. **需求诊断** - 引导您提供项目背景、最终交付日及当前痛点
2. **逻辑构建** - 协助您梳理任务清单，确认任务间的依赖关系
3. **生成方案** - 输出项目进度跟踪核心表和风险与关键路径分析报告
4. **监控与调整** - 根据您反馈的执行情况，提供进度更新和纠偏策略

请告诉我:
1. 项目的目标是什么？最终截止日期（Deadline）是哪天？
2. 目前有哪些核心的大块任务（Milestones）？
3. 项目团队规模及主要资源限制是什么？`,
      "1404": `你好！我是拥有50年一线项目落地经验的项目沟通计划大师。

在复杂的项目环境中，我能帮您解决信息孤岛、干系人利益冲突、汇报无效或即时沟通混乱等问题。

**我能为您做什么:**
1. **诊断痛点** - 快速分析您当前项目中存在的沟通阻碍
2. **制定策略** - 输出包含干系人分析、沟通矩阵、汇报机制的完整计划
3. **提供工具** - 生成具体的邮件模版、会议议程、话术脚本或文档模板
4. **风险预警** - 预判沟通风险，并给出预防措施

**我的工作流程:**
1. **引导阶段** - 询问您的项目类型、当前角色及面临的最大沟通挑战
2. **分析阶段** - 根据您的输入，进行干系人画像分析和痛点诊断
3. **方案生成** - 输出《项目沟通管理计划表》和关键场景的话术
4. **迭代优化** - 根据您的反馈调整沟通策略的语气和手段

现在，请描述您正在经历的"沟通噩梦"或项目背景。`,
      "1405": `您好！我是您的项目验收报告专家，拥有50年丰富的项目落地和验收经验。👋

我可以为您提供以下专业服务：
- ✅ 编制各类项目验收报告（工程、软件、咨询等）
- ✅ 制定科学的验收标准和指标体系
- ✅ 设计完整的验收工作流程
- ✅ 提供验收文档模板和检查清单
- ✅ 诊断项目问题并提供解决方案
...
**我的工作流程**：需求分析 → 方案设计 → 内容编制 → 审核优化 → 交付指导

请告诉我：
1. 您的项目类型是什么？（如：建筑工程/软件开发/设备采购等）
2. 目前处于什么验收阶段？（如：初验/终验/专项验收等）
3. 您最需要我帮助的是？（如：编制验收报告/制定验收标准/设计验收流程等）

让我们开始为您的项目打造专业、规范的验收方案！🎯`,
      "1406": `您好！我是拥有50年实战经验的项目后评价报告专家。

**我能为您做什么:**
1. **信息重构** - 引导您提供关键项目信息，将碎片化的数据重构为结构化的评价内容
2. **深度分析** - 不仅描述现象，更要进行原因分析（Why）和影响分析（Impact）
3. **专业输出** - 生成符合行业标准、逻辑严密、措辞专业的项目后评价报告
4. **价值提炼** - 精准总结经验教训，为未来的决策提供参考建议

**我的工作流程:**
1. **需求收集** - 询问您项目的基本情况（项目类型、预算规模、起止时间、核心目标）
2. **大纲确认** - 根据项目类型推荐合适的后评价报告大纲
3. **细节填充** - 分模块引导您输入信息
4. **草稿生成** - 基于收集的信息，撰写各章节内容
5. **复盘优化** - 生成"结论与建议"章节，并对全文进行逻辑校验

请告诉我您希望评价的项目名称及大致类型（例如：软件开发、工程建设、市场营销活动等），我们将开始第一步的工作。`
    };
    return welcomeMessages[templateId] || welcomeMessages["1401"];
  };

  // 初始化欢迎消息（所有项目管理子类型）
  useEffect(() => {
    const projectManagementTemplateIds = ["1401", "1402", "1403", "1404", "1405", "1406"];
    if (projectManagementTemplateIds.includes(templateId) && messages.length === 0) {
      setMessages([{
        id: 'welcome',
        role: 'assistant',
  ...
  const getApiEndpoint = (templateId: string): string => {
    switch (templateId) {
      case "1401": return "/api/project-management/startup-report";
      case "1402": return "/api/project-management/plan";
      case "1403": return "/api/project-management/progress-tracking";
      case "1404": return "/api/project-management/communication-plan";
      case "1405": return "/api/project-management/acceptance-report";
      case "1406": return "/api/project-management/post-evaluation";
      default: return "/api/project-management/startup-report";
    }
  };
  ...
  const handleCommunicationNewConversation = () => {
    ...
    // 所有项目管理子类型：重置消息列表为对应的欢迎消息
    const projectManagementTemplateIds = ["1401", "1402", "1403", "1404", "1405", "1406"];
    if (projectManagementTemplateIds.includes(templateId)) {
      setMessages([{
        id: 'welcome-' + Date.now(),
        role: 'assistant',
        content: getWelcomeMessage(templateId),
  ...
  return (
    <div className="flex h-[calc(100vh-56px)]">
      {["1401", "1402", "1403", "1404", "1405", "1406"].includes(templateId) ? (
        /* 所有项目管理子类型：统一使用对话模式UI */
